...
class Account {
  ...
  cards(cardId) {
    if (cardId) {
      return utils
        .callApi(this, 'getCard', cardId)
        .then(card => new Card(this, card));
    }

    return utils
      .callApi(this, 'getCards')
  ...
  invitations(emails) {
    if (emails) {
      ...
      return Promise.each(emails, email =>
        utils.callApi(this, 'sendInvitations', email)
      );
    }
  ...
  memo(smartLinkId, memo) {
    return utils
      .callApi(this, 'getTransactionMeta', smartLinkId)
      .then(meta =>
        utils.callApi(this, 'createOrUpdateMemo', { smartLinkId, meta, memo })
      );
  }
  ...
  statement(id, pdf) {
    return Promise.try(() => {
      ...
      return utils.callApi(this, 'getStatement', { id, pdf }).then(result => ({
        id,
        type: pdf ? 'pdf' : 'base64',
  ...
  stats(from, to, interval) {
    return Promise.try(() => {
      ...
      from = moment(
        from instanceof Date ? from.getTime() / 1000 : from,
        'X'
      ).startOf(interval);
      to = moment(to instanceof Date ? to.getTime() / 1000 : to, 'X').endOf(
        interval
      );

      while (from < to) {
        const tempTo = moment(from)
          .add(1, interval)
          .endOf(interval);

        request.push(
          utils.callApi(this, 'getStats', {
            from: from.valueOf(),
            to: tempTo.valueOf()
          })
        );
