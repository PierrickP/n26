...
class Account {
  ...
  cards(cardId) {
    ...
    return utils.callApi(this, 'getCards')
      .then(function (cards_raw) {
        var cards = cards_raw.map(card_raw => new Card(account, card_raw));

        return {
          paging: null,
          data: cards.filter(card => /^M_/.test(card.n26Status)),
          data_legacy: cards.filter(card => !/^M_/.test(card.n26Status))
        }
      });
  }
   ...
   *
   */
  categoryStats(from, to) {
    return Promise.try(() => {
      if (!from || !to) {
        throw new Error('MISSING_PARAMS');
      }

      from = (from instanceof Date) ? from.getTime() : from*1000;
      to = (to instanceof Date) ? to.getTime() : to*1000;

      return utils.callApi(this, "getCategoryStats", {from: from, to: to});
    });
  }
