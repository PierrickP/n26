...
class Card {
  constructor(account, card) {
    ...
    this.exceetExpressCardDeliveryTrackingId = card.exceetExpressCardDeliveryTrackingId;
  ...
  block() {
    return utils.callApi(this.account, 'blockCard', this.id)
      .tap(() => { this.status = /^M_/.test(this.status) ? 'M_DISABLED' : 'BLOCKED' ; });
  }
  ...
  unblock() {
    return utils.callApi(this.account, 'unblockCard', this.id)
      .tap(() => { this.status = /^M_/.test(this.status) ? 'M_ACTIVE' : 'OPEN'; });
  }
}
