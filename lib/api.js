...
module.exports = {
    ...
    return request
      .post({
        ...
        headers: {
          ...
          'User-Agent':
            'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.109 Safari/537.36' // eslint-disable-line max-len
        }
      })
      .catch(errorHandler);
  },
  ...
  getStats(token, opt) {
    return request
      .get({
        url: `${api}/api/accounts/stats?type=acct&from=${opt.from}&to=${
          opt.to
        }&numSlices=1`,
        json: true,
        headers: {
  ...
  getTransaction(token, opts) {
    ...
    return request
      ...
      .then(transaction => {
        ...
        return that
          .getTransactionMeta(token, transaction.smartLinkId)
          .then(meta => {
