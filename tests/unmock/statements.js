...
const statementsProperties = ['id', 'month', 'url', 'visibleTS', 'year'];
...
describe('Statements', () => {
  it('should return statements', () => {
    return global.n26.statements().then(statements => {
      ...
      console.log(
        `\tLast statements for ${statements[0].month}/${statements[0].year}`
      );
    });
  });
  ...
  if (global.CONFIG.options.indexOf('Statement') === -1) {
    ...
    it('should get last statement file', function() {
      // eslint-disable-line func-names
      this.timeout(25000);

      return global.n26
        .statements()
        .then(statements => statements[0].id)
        .then(statementId => {
          return Promise.all([
